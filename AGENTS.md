# Magic Web UI - Agent 项目文档

## 项目概述

Magic Web UI 是 Magic Agent 的全功能 Web Dashboard，提供 AI 聊天会话管理、平台渠道集成（8 个平台）、使用分析、定时任务、模型管理、配置文件管理和集成终端。
...
| CLI 入口 | `bin/magic-web-ui.mjs` | 全局 CLI 命令 |
...
3. **Magic Agent 依赖**：dev 模式依赖 `magic` CLI 在 `$PATH` 中
...
3. **后端启动失败**：检查 Magic CLI 是否可用，以及 8648 端口是否被占用

## go-magic 后端适配

Magic Web UI 支持通过 BFF 适配层连接 go-magic 后端。适配层位于：
...
Vite dev server 配置了 API 代理，可以直接代理到 go-magic 后端：
...
server: {
  proxy: {
    '/api/magic': {
      target: process.env.MAGIC_API_URL || 'http://localhost:5000',
      changeOrigin: true,
...
**端口说明：**
- go-magic server: 5000 端口
- Vite dev/preview: 5001 端口（避免端口冲突）
