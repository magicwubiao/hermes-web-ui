...
import { listConversationSummaries, getConversationDetail } from '../../services/magic/conversations'
import { listConversationSummariesFromDb, getConversationDetailFromDb } from '../../db/magic/conversations-db'
import { listSessionSummaries, searchSessionSummaries, getUsageStatsFromDb, getSessionDetailFromDb } from '../../db/magic/sessions-db'
import {
...
} from '../../db/magic/session-store'
import { ExportCompressor } from '../../lib/context-compressor/export-compressor'
import { getGatewayManagerInstance } from '../../services/gateway-bootstrap'
import { deleteUsage, getUsage, getUsageBatch, getLocalUsageStats } from '../../db/magic/usage-store'
import type { LocalUsageStats, UsageStatsModelRow, UsageStatsDailyRow } from '../../db/magic/usage-store'
import { getModelContextLength } from '../../services/magic/model-context'
import { getActiveProfileName } from '../../services/magic/hermes-profile'
import { getGroupChatServer } from '../../routes/magic/group-chat'
import { logger } from '../../services/logger'
import type { ConversationSummary } from '../../services/magic/conversations'
...
export async function setWorkspace(ctx: any) {
  ...
  if (useLocalSessionStore()) {
    const { updateSession, getSession, createSession } = await import('../../db/magic/session-store')
    const { getActiveProfileName } = await import('../../services/magic/hermes-profile')
...
export async function getConversationMessagesPaginated(ctx: any) {
  ...
  if (useLocalSessionStore()) {
    const { getSessionDetailPaginated } = await import('../../db/magic/session-store')
